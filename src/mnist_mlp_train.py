...
def load_mnistc_datasets(data_path):
    test_ds_images_u8=np.load(os.path.join(data_path,'shotnoise'))
    train_ds_images_u8, train_ds_labels = tfds.as_numpy(
        # tfds.load("mnist", split="train", batch_size=-1, as_supervised=True))
        tfds.load("mnist_corrupted/shot_noise", split="train", batch_size=-1, as_supervised=True))

    test_ds_images_u8, test_ds_labels = tfds.as_numpy(
        # tfds.load("mnist", split="test", batch_size=-1, as_supervised=True))
        tfds.load("mnist_corrupted/shot_noise", split="test", batch_size=-1, as_supervised=True))

    train_ds = {"images_u8": train_ds_images_u8, "labels": train_ds_labels}
    test_ds = {"images_u8": test_ds_images_u8, "labels": test_ds_labels}
    return train_ds, test_ds

def load_mnistc_datasets():
    train_ds_images_u8, train_ds_labels = tfds.as_numpy(
        # tfds.load("mnist", split="train", batch_size=-1, as_supervised=True))
        tfds.load("mnist_corrupted/shot_noise", split="train", batch_size=-1, as_supervised=True))
...
def main():
    ...
    parser.add_argument("--dataset",type=str,default="mnist",choices=["mnist", "mnist-corrupted", "both"])
    ...
    with wandb.init(
        ...
        artifact = wandb.Artifact(args.dataset+"-mlp-weights", type="model-weights")
