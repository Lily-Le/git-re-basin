...
if __name__ == "__main__":
  ...
  with wandb.init(
    ...
    artifact = wandb.Artifact("mnist_mlp_activation_matching",
                              type="permutation",
                              metadata={
                                  "dataset": "mnist",
                                  "model": "mlp",
    ...
    for layer in all_layers:
      ...
      fig.tight_layout()
      plt.savefig(f"mnist_mlp_activation_inner_products_{layer}.png", dpi=300)
      plt.savefig(f"mnist_mlp_activation_inner_products_{layer}.eps")
    ...
    for layer in all_layers:
      ...
      fig.tight_layout()
      plt.savefig(f"mnist_mlp_activation_correlations_{layer}.png", dpi=300)
      plt.savefig(f"mnist_mlp_activation_correlations_{layer}.eps")
    ...
    for layer in all_layers:
      ...
      fig = plt.figure()
      plt.title(f"MLP on MNIST ({layer})")
      ...
      plt.legend()
      plt.tight_layout()
      plt.savefig(f"mnist_mlp_activation_scatter_{layer}.png", dpi=300)
      plt.savefig(f"mnist_mlp_activation_scatter_{layer}.eps")
    ...
    fig = plot_interp_loss(config.load_epoch, lambdas, train_loss_interp_naive,
                           ...
                           test_loss_interp_clever)
    plt.savefig(f"mnist_mlp_interp_loss_epoch{config.load_epoch}_activation_matching.png", dpi=300)
    plt.savefig(f"mnist_mlp_interp_loss_epoch{config.load_epoch}_activation_matching.eps")
    ...
    fig = plot_interp_acc(config.load_epoch, lambdas, train_acc_interp_naive, test_acc_interp_naive,
                          train_acc_interp_clever, test_acc_interp_clever)
    plt.savefig(f"mnist_mlp_interp_accuracy_epoch{config.load_epoch}_activation_matching.png",
                dpi=300)
    plt.savefig(f"mnist_mlp_interp_accuracy_epoch{config.load_epoch}_activation_matching.eps")
